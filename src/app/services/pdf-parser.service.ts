...
export class PdfParserService {
  ...
  private getWorkerBlobUrl(): Promise<string> {
    if (!this.workerBlobUrlPromise) {
      ...
      console.log('[PDF] fetching worker from', src);
      this.workerBlobUrlPromise = fetch(src)
        .then(r => {
          console.log('[PDF] worker fetch response:', r.status, r.ok);
          if (!r.ok) throw new Error(`Worker fetch: HTTP ${r.status}`);
          return r.text();
        })
        .then(code => {
          console.log('[PDF] worker code length:', code.length);
          const blobUrl = URL.createObjectURL(
            new Blob([code], { type: 'application/javascript' }),
          );
          console.log('[PDF] worker blob URL created:', blobUrl.slice(0, 40));
          return blobUrl;
        })
        .catch(err => {
          console.error('[PDF] worker setup failed:', err);
          this.workerBlobUrlPromise = null; // allow retry
          throw err;
        });
    }
  ...
  async parsePdf(file: File): Promise<ParsedSong[]> {
    console.log('[PDF] parsePdf start — file:', file.name, 'size:', file.size, 'type:', file.type);

    console.log('[PDF] importing pdfjs-dist...');
    const pdfjsLib = await import('pdfjs-dist');
    console.log('[PDF] pdfjs-dist loaded, version:', (pdfjsLib as any).version ?? 'unknown');

    console.log('[PDF] setting up worker blob URL...');
    const workerSrc = await this.getWorkerBlobUrl();
    pdfjsLib.GlobalWorkerOptions.workerSrc = workerSrc;
    console.log('[PDF] workerSrc set');

    console.log('[PDF] reading file as ArrayBuffer...');
    const ab = await file.arrayBuffer();
    console.log('[PDF] arrayBuffer ready, byteLength:', ab.byteLength);

    console.log('[PDF] calling getDocument...');
    const loadTask = pdfjsLib.getDocument({ data: ab });
    console.log('[PDF] getDocument task created, awaiting promise with 20s timeout...');

    let timedOut = false;
    const pdf = await Promise.race([
      loadTask.promise.then(p => { console.log('[PDF] getDocument resolved, pages:', p.numPages); return p; }),
      new Promise<never>((_, reject) =>
        setTimeout(() => {
          timedOut = true;
          console.error('[PDF] 20s timeout reached — worker likely failed to start');
    ...
    try {
      ...
      if (info?.['Subject'] === 'WorshipToolkit') {
        ...
        console.log('[PDF] WorshipToolkit PDF detected, col2X:', wtCol2X);
      }
    } catch (e) {
      console.warn('[PDF] getMetadata failed (non-fatal):', e);
    }
    ...
    for (let p = 1; p <= pdf.numPages; p++) {
      console.log('[PDF] getting page', p);
      const page = await pdf.getPage(p);
      const vp = page.getViewport({ scale: 1 });
      console.log('[PDF] page', p, 'viewport:', vp.width.toFixed(0), 'x', vp.height.toFixed(0));
      const tc = await page.getTextContent();
      console.log('[PDF] page', p, 'text items:', tc.items.length);
      const items: RawItem[] = [];
      ...
      console.log('[PDF] page', p, 'non-empty items:', items.length);
    ...
    console.log('[PDF] lines per page:', pageLinesList.map(l => l.length));
