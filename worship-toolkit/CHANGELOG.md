...
### Added
- **Latin Easter Egg** — hidden in the user manual: a "🏛️ Switch to Latin" pill button flips every UI label in the app to humorous Latin (buttons, headers, card text, modal wording, and more). Activating shows a "Modus Latinus Activatus 🏛️" toast; deactivating shows "Modus Latinus Deactivatus". Latin mode is saved with each set and restored when that set is loaded.
- **Cantus Secretus** — with Latin mode active, a secret collapsible section appears in the manual page containing the full Latin lyrics of *Unus Angelus Alae* (Nobuo Uematsu, 1997).
